...
const { requirePermission, httpError } = require('./ops-access.cjs');
const { firestoreGet, firestoreRunQuery, firestoreCommit, docName, docFields, toFields } = require('./ops-firestore.cjs');
...
async function handle(req, actor, path, method = String(req.method || "GET").toUpperCase()) {
    if (method === "GET" && path[0] === "leads" && path.length === 1) {
      return reply(200, await listLeads(req, actor));
    }
    if (method === "POST" && path[0] === "leads" && path.length === 1) {
      requirePermission(actor, "edit_records");
      return reply(201, await createLead(req, actor));
    }
    if (method === "GET" && path[0] === "leads" && path[1]) {
      const lead = await getLead(path[1], actor);
      if (!lead) return reply(404, { error: "not_found", message: "Lead not found." });
      return reply(200, lead);
    }
    if (method === "PATCH" && path[0] === "leads" && path[1] && path.length === 2) {
      requirePermission(actor, "edit_records");
      return reply(200, await patchLead(path[1], req, actor));
    }
    if (method === "PUT" && path[0] === "leads" && path[1] && path[2] === "status") {
      requirePermission(actor, "change_status");
      return reply(200, await updateLeadStatus(path[1], req, actor));
    }
    if (method === "POST" && path[0] === "leads" && path[1] && path[2] === "notes") {
      requirePermission(actor, "edit_records");
      return reply(201, await addLeadNote(path[1], req, actor));
    }
    if (method === "GET" && path[0] === "tasks" && path.length === 1) {
      return reply(200, await listTasks(req, actor));
    }
    if (method === "PATCH" && path[0] === "tasks" && path[1] && path.length === 2) {
      requirePermission(actor, "edit_records");
      return reply(200, await patchTask(path[1], req, actor));
    }
    if (method === "GET" && path[0] === "audit" && path.length === 1) {
      return reply(200, await listAudit(req, actor));
    }
    if (method === "GET" && PLANNED_RESOURCES.has(path[0])) {
      return reply(200, plannedResource(path[0]));
    }

    return reply(404, { error: "not_found", message: "Unknown operations endpoint." });
}
