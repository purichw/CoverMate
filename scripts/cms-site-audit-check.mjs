...
try {
  ...
  const applyImage=async()=>{await page.getByRole('button',{name:'ใช้รูปนี้ใน draft',exact:true}).click();await page.locator('.cm-media-dialog').waitFor({state:'detached'});};
  await openCrop(iconPath);await prepareImage();
  await page.getByRole('button',{name:'ขยาย',exact:true}).click();
  await page.getByRole('button',{name:'เลื่อนขวา',exact:true}).click();
  await shot('admin-image-crop-desktop.png');
  uploadFailure=true;
  await page.getByRole('button',{name:'ใช้รูปนี้ใน draft',exact:true}).click();
  await page.locator('.cm-media-error').getByText('บันทึกไม่สำเร็จ รูปเดิมยังไม่เปลี่ยน').waitFor();
  ...
  await openCrop('seo.image');await prepareImage();await page.getByRole('radio',{name:'แสดงรูปเต็ม',exact:true}).check();await applyImage();await save();
  ...
  assert.equal(contract.cmsGet(draft.config,iconPath),iconURL);
  await openCrop(iconPath);await page.locator('.cm-media-dialog').getByRole('button',{name:'ล้างรูป',exact:true}).click();await save();assert.equal(contract.cmsGet(draft.config,iconPath),'');
  ...
  await page.getByRole('button',{name:'ปิดแผง Admin',exact:true}).click();
