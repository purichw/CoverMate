...
edited.sections.find(s => s.id === 'faq').items.find(item => item.sourceGuideId).th.a = 'ผมพร้อมช่วยครับ';
...
assert.equal(v2.cmsContentVersion, contract.CMS_CONTENT_VERSION);
...
const identifiers = createSeoModel(normalized, { lang: 'en', path: '/motor' }).graph['@graph'][1].identifier;
...
const graph = createSeoModel(normalized, { lang: 'en', path: '/motor' }).graph['@graph'];
assert.ok(!('areaServed' in graph[1]) && !('knowsAbout' in graph[1]));
assert.ok(!graph.some(entity => entity['@type'] === 'Service'), 'Blank service metadata removes the incomplete Service entity');
