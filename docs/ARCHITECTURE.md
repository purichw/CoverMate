...
CoverMate is a Vercel-hosted static export. The deployed visitor file is still
`index.html`, but its source now lives in `src/visitor/` and is generated by
`scripts/generate-visitor-bundle.mjs`. Admin pages remain source-authored static
HTML/JS surfaces plus shared helpers.
...
production host `covermate.vercel.app` always resolves to production, even if a
