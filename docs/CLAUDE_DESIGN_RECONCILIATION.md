...
| Admin launcher | `/admin` remains after login as the Admin Portal shell with four modules: `Operations`, `Website content`, `Analytics`, and `Settings`. Inside `Website content`, the editing entry is unified: `Edit the words` opens the editor and `Tools -> Panel` opens the control panel. Do not add a separate `Arrange and customise` card. |
